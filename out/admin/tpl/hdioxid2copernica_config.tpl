...
<div id="hdio2c">
    ...
    <h1>[{oxmultilang ident="hdioxid2copernica_config" }] <span data-bind="text:curStep"></span></h1>
    <div id="accountPanel" data-bind="visible:curStep() == 'Account'">
        [{oxmultilang ident="SHOP_MODULE_hdio2c_host" }] <br/>
        <input type="text" data-bind="value:host"/><br/><br/>
        [{oxmultilang ident="SHOP_MODULE_hdio2c_user" }] <br/>
        <input type="text" data-bind="value:user"/><br/><br/>
        [{oxmultilang ident="SHOP_MODULE_hdio2c_pass" }] <br/>
        <input type="password" data-bind="value:password"/><br/><br/>
        [{oxmultilang ident="SHOP_MODULE_hdio2c_acc" }] <br/>
        <input type="text" data-bind="value:account"/><br/><br/>
        <p data-bind="html:loginStatus">
        </p>
        <button data-bind="enable: loginOK(), click: function(){ConfigModel.curStep('Datenbank und Felder')}" >[{oxmultilang ident="HDIO2C_NEXT"}]</button>
    </div>
    <div id="databasePanel" data-bind="visible:curStep() == 'Datenbank und Felder'">
        <input type="radio" name="newAuto" value="0" checked data-bind="checked: newAuto"/>[{oxmultilang ident="HDIO2C_DBF_NEWAUTO1" }]<br/>


        <input type="radio" name="newAuto" value="1" data-bind="checked: newAuto"/>[{oxmultilang ident="HDIO2C_DBF_NEWAUTO2 }]

        <div data-bind="visible: newAuto() == 0">
            [{oxmultilang ident="HDIO2C_DBNAME" }] <br><input type="text" data-bind="value: newDbName, valueUpdate: 'afterkeydown'" /> <button data-bind="enable: newDbName() != '', click: createAuto">[{oxmultilang ident="HDIO2C_CREATE_DATABASE" }]</button>
        </div>



        <table data-bind="if: newAuto() == 1"><tr><td>
                    <h2>[{oxmultilang ident="HDIO2C_DATABASE"}]:</h2>
                    <select data-bind="options: databases, optionsText: 'name',optionsCaption:'Bitte auswählen...', value: database().account().collection"></select><span class="wait" data-bind="visible:waiting().database() || database().account().waiting() "></span>
                    <div class="newPanel" data-bind="visible: checkNewCollection('account')">
                        [{oxmultilang ident="HDIO2C_NEWDB" }]<br/> <input type="text" data-bind="value: newDatabase, valueUpdate: 'afterkeydown'" />
                        <button data-bind="enable: newDatabase() != '',click: createDatabase">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div>

                    <br/>
                    <table data-bind="template:{name: 'fields-template', foreach: database().account().fieldMatching}">
                    </table>
                </td>

                <td>

                    <h2>[{oxmultilang ident="HDIO2C_ORDER" }]</h2>
                    <select data-bind="options: database().collections, optionsText:'name',optionsCaption:'Bitte auswählen...', enable:database().collections().length > 0, value:database().orders().collection"></select><span class="wait" data-bind="visible:waiting().collections() || database().orders().waiting()"></span>
                    <div class="newPanel" data-bind="with:database().orders, visible: checkNewCollection('orders')">
                        [{oxmultilang ident="HDIO2C_NEWCOLLECTION" }]<br/> <input class="newInput" type="text" data-bind="value: newCollectionName,attr:{def: newCollectionName}, valueUpdate: 'afterkeydown'" />
                        <button data-bind="enable: newCollectionName != '',click:  $root.createCollection">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div>
                    <table data-bind="template:{ name: 'fields-template', foreach: database().orders().fieldMatching}">
                    </table>

                </td><td>

                    <h2>[{oxmultilang ident="HDIO2C_ORDERITEMS" }]</h2>
                    <select data-bind="options:database().collections, optionsText:'name',optionsCaption:'Bitte auswählen...', enable:database().collections().length > 0, value:database().orderItems().collection"></select><span class="wait" data-bind="visible:waiting().collections() || database().orderItems().waiting()"></span>
                    <div class="newPanel" data-bind="with: database().orderItems,visible: checkNewCollection('orderItems')">
                        [{oxmultilang ident="HDIO2C_NEWCOLLECTION" }]<br/> <input class="newInput" type="text" data-bind="value: newCollectionName,attr:{def: newCollectionName}, valueUpdate: 'afterkeydown'" />
                        <button data-bind="enable: newCollectionName != '',click: $root.createCollection">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div> 
                    <table data-bind="template:{name: 'fields-template', foreach: database().orderItems().fieldMatching}">
                    </table>
                </td><td>

                    <h2>[{oxmultilang ident="HDIO2C_BASKET" }]</h2>
                    <select data-bind="options: database().collections, optionsText:'name',optionsCaption:'Bitte auswählen...', enable:database().collections().length > 0, value:database().basket().collection"></select><span class="wait" data-bind="visible:waiting().collections() || database().basket().waiting()"></span>
                    <div class="newPanel" data-bind="with: database().basket, visible: checkNewCollection('basket')">
                        [{oxmultilang ident="HDIO2C_NEWCOLLECTION" }]<br/> <input class="newInput" type="text" data-bind="value: newCollectionName, attr: {def: newCollectionName}, valueUpdate: 'afterkeydown'" />
                        <button data-bind="enable: newCollectionName != '', click: $root.createCollection">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div>
                    <table data-bind="template:{ name: 'fields-template', foreach: database().basket().fieldMatching}">
                    </table>
                </td><td>

                    <h2>[{oxmultilang ident="HDIO2C_ADDRESSES" }]</h2>
                    <select data-bind="options:database().collections, optionsText:'name',optionsCaption:'Bitte auswählen...', enable:database().collections().length > 0, value:database().addresses().collection"></select><span class="wait" data-bind="visible:waiting().collections() || database().addresses().waiting()"></span>
                    <div class="newPanel" data-bind="with: database().addresses, visible: checkNewCollection('addresses')">
                        [{oxmultilang ident="HDIO2C_NEWCOLLECTION" }]<br/> <input class="newInput" type="text" data-bind="value: newCollectionName, attr:{def: newCollectionName}, valueUpdate: 'afterkeydown'" />
                        <button data-bind="enable: newCollectionName != '', click: $root.createCollection">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div>
                    <table data-bind="template:{name: 'fields-template', foreach: database().addresses().fieldMatching}">
                    </table>
                </td></tr><tr>
                <td>

                    <h2>[{oxmultilang ident="HDIO2C_DATABASE"}]:</h2>
                    <select data-bind="options: databases, optionsText: 'name',optionsCaption:'Bitte auswählen...', value: database().products().collection"></select><span class="wait" data-bind="visible:waiting().database() || database().products().waiting() "></span>
                    <div class="newPanel" data-bind="visible: checkNewCollection('products')">
                        [{oxmultilang ident="HDIO2C_NEWDB" }]<br/> <input type="text" data-bind="value: newDatabase, valueUpdate: 'afterkeydown'" />
                        <button data-bind="enable: newDatabase() != '',click: createDatabase">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div>

                    <br/>
                    <table data-bind="template:{name: 'fields-template', foreach: database().products().fieldMatching}">
                    </table>
        ...
        <button data-bind="enable: allowSave, click: saveDatabase">[{oxmultilang ident="HDIO2C_SAVE"}]</button>
    </div>

    <!-- TEMPLATE FOR A FIELD ROW -->
    <script type="text/html" id="fields-template">
        <tr>
            <td data-bind="text: name"></td>
            <td>
                <select class="field" data-bind="options:$root.database()[collection]().fields, optionsText:'name', optionsCaption:'Bitte auswählen...', value:match, enable:$root.database()[collection]().fields().length >0"></select>
                <div data-bind="if: match() && match().id == -1">
                    <div class="newPanel" >
                        [{oxmultilang ident="HDIO2C_NEWFIELD" }]<br/> <input class="newInput" type="text" data-bind="value: newField, attr:{def: newField}, valueUpdate: 'afterkeydown'" /><button data-bind="click: $root.newField">[{oxmultilang ident="HDIO2C_NEXT"}]</button>
                    </div>
                </div>
            </td>
            <td></td>
    ...
    database: ko.observable({
        ...
        basket: ko.observable(
            ...
            fieldMatching: ko.observableArray([
                    ...
                    {internal:"BasketType", name: "[{oxmultilang ident='HDIO2C_basket_BasketType' }]", type: "text", newField: "Basket_Type", match: ko.observable(), collection: "basket" }
        ...
        orderItems: ko.observable(
            ...
            fieldMatching: ko.observableArray([
                    ...
                    {internal:"OrderID", name: "[{oxmultilang ident='HDIO2C_orderItems_OrderID' }]", type: "text", newField: "Order_ID", match: ko.observable(), collection: "orderItems" },
